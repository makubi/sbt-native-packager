...
trait DockerKeys {
  ...
  val dockerTarget = SettingKey[String]("docker-target", "Defines target used when building and publishing Docker image")
  val dockerBuildCmd = SettingKey[Seq[String]]("dockerBuildCmd", "Build command used for local publishing")
